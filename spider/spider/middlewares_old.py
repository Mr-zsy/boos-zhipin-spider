...
class ProxyMiddleWare(object):
    ...
    @classmethod
    def sqProxy(cls):
        proxy = requests.get('http://192.168.43.238:8080/proxy')
        proxy_json = json.loads(proxy.text)
        return proxy_json
    ...
    # def process_request(self, request, spider):
        ...
        # request.meta['proxy'] = 'http://1.195.124.19:49132'

    def process_response(self, request, response, spider):
        print('－－－－respoonse.meta－－－－',response.meta)
        ...
        if response.status != 200:
            ...
            return request
        headerElement = response.css('.error-content').extract_first()
        if headerElement:
            print('－－－－ip异常！－－－－')
            # request.meta['proxy'] = self.getProxy()
            request.meta['proxy'] = self.sqProxy()
            return request
